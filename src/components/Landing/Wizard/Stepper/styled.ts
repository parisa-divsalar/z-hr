...
export const StepItem = styled(Box)<{ active?: boolean }>(({ theme }) => ({
  display: 'flex',
  alignItems: 'center',
...
export const StepCircle = styled(Box)<{ active?: boolean }>(({ theme, active }) => ({
  width: 40,
  height: 40,
  borderRadius: '50%',
...
export const Divider = styled('div')<{ active?: boolean }>(({ theme, active }) => ({
  flex: '0 0 50px',
  height: 2,
