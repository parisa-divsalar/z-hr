...
const VoiceRecording = ({
  ...
  const cleanup = useCallback(() => {
    ...
    // stop audio playback and release ref
    if (audioRef.current) {
      try {
    ...
    // revoke internal url only on full cleanup (not while still using)
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        // revoke any previous internal URL (we keep external initialAudioUrl)
        if (audioUrl && isInternalUrl) {
          try {
            URL.revokeObjectURL(audioUrl);
          } catch (e) {}
  ...
  const stopRecording = useCallback(() => {
    ...
    // also stop mic tracks proactively
    if (streamRef.current) {
  ...
  const playRecording = useCallback(async () => {
    ...
    try {
      ...
      if (!audioRef.current) {
        ...
        // metadata event (may be unreliable on some combos)
        audio.onloadedmetadata = () => {
          if (audioRef.current && isFinite(audioRef.current.duration) && audioRef.current.duration > 0) {
            setAudioDuration(audioRef.current.duration);
            // compute wave bars (20 bars per second)
            const bars = Math.max(20, Math.floor(audioRef.current.duration * 20));
            setWaveBarsCount(bars);
          }
        };

        // more reliable event for real usable duration
        ...
        // fallback: sometimes neither fires immediately for blob urls, try small timeout check
        ...
        // clear fallback when we unmount or next time
        const clearFallback = () => clearTimeout(fallbackTimer);
        // attach cleanup of fallback on canplaythrough or loadedmetadata
        audio.oncanplaythrough = (() => {
          if (audioRef.current && isFinite(audioRef.current.duration) && audioRef.current.duration > 0) {
            setAudioDuration(audioRef.current.duration);
            const bars = Math.max(20, Math.floor(audioRef.current.duration * 20));
        ...
        // note: we already set oncanplaythrough above, this is to ensure fallback cleared
      }
  ...
  /** when audioUrl changes externally (like initialAudioUrl), ensure audioRef reset */
  useEffect(() => {
    if (audioUrl && audioRef.current) {
      // if audioRef has previous src, reset so new src attached when user plays
      try {
        audioRef.current.pause();
        audioRef.current.src = '';
      } catch (e) {}
