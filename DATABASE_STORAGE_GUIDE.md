...
services:
  postgres:
    ...
    volumes:
      ...
      - ./sql/postgres.sql:/docker-entrypoint-initdb.d/init.sql
    healthcheck:
...
برای ساخت سریع دیتابیس محلی، این فایل‌ها اضافه شده‌اند:
...
- اگر PostgreSQL داری:
  - داخل دیتابیس اجرا کن:
    - فایل: `sql/postgres.sql`

- اگر با Docker می‌خواهی:
  - در `DATABASE_STORAGE_GUIDE.md` نشان داده شده که:
    - فایل `sql/postgres.sql` به عنوان init به کانتینر وصل شده
  - اجرا:
    - `docker-compose up -d`
